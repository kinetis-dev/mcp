...
/**
 * The MCP server as a command. It reads JSON-RPC one message per line from
 * stdin and writes one per line back, so driving it with in-memory streams
 * exercises the whole thing — project-root resolution, registry
 * construction, dispatcher wiring, and the transport loop — and stops when
 * the input is exhausted rather than blocking on a terminal.
 */
final class McpServeCommandTest extends TestCase
{
    ...
    public function test_initialize_selects_this_servers_one_revision(): void
    {
    ...
    /**
     * A project with nothing to serve still answers cleanly — empty lists,
     * not errors. In a real install the docs resources appear here too,
     * contributed through this package's own extra.kinetis scan root, which
     * a scratch directory with no installed.json cannot carry.
     */
    ...
    /**
     * The notification a real client sends right after initialize is
     * answered with nothing, so one response comes back for these two
     * messages rather than two.
     */
    public function test_the_initialized_notification_is_consumed_without_a_response(): void
    {
        $responses = $this->serve([
    ...
    private function serve(array $messages): array
    {
        ...
        // The binding the package bootstrap makes in a real install, here
        // pointed at this test's own scratch project root.
        $app = new AppScope();
